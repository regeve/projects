 ...
 function animationPipeline() {
  ...
  self.createQuestion = function() {
	...
	for(var i=0;i<4;i++) {
		...
		} while (rand == index || isInArray(answers, notes[rand]));
		if (i == correctAnswer) {
			answers[i] = notes[index];
		} else {
			answers[i] = notes[rand];
		}
	}
   ...
   self.windowWasResized = function() {
    stage.style.height = (h -single_game_time) +'px';
    stage.style.width = (w - single_game_time) + 'px';
   };
   ...
   self.setupUserInterfaceWithData = function() {
	...
	var a = new Audio(ques.audio_path);
    t[0].innerHTML = '<audio id="audio1" controls="controls"><source id="mp3Source" type="audio/mp3"></source></audio>';
    var audio = document.getElementById('audio1');
	var source = document.getElementById('mp3Source');
	source.src = ques.audio_path;
	audio.load();
	audio.play();
